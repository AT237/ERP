...
import { Upload, Trash2, Download, File, FileText, FileImage, FileVideo, FileAudio, FileArchive, FileSpreadsheet, Loader2 } from "lucide-react";
...
export function ProjectFilesTab({ projectId }: ProjectFilesTabProps) {
  ...
  const queryKey = ["/api/attachments", "project-file", projectId];
  ...
  const uploadFiles = useCallback(async (fileList: File[]) => {
    ...
    try {
      for (const file of fileList) {
        ...
        await apiRequest("POST", `/api/attachments/project-file/${projectId}`, {
          fileName: file.name,
      ...
      queryClient.invalidateQueries({ queryKey });
      toast({ title: `${fileList.length} bestand(en) geüpload` });
    } catch (err) {
      toast({ title: "Upload mislukt", variant: "destructive" });
    } finally {
      setUploading(false);
    }
  }, [projectId, files.length, queryKey, toast]);
  ...
  if (!projectId) {
    return (
      <div className="text-center py-8 text-gray-500 text-sm">
        Sla het project eerst op om bestanden te kunnen uploaden.
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      >
        ...
        <p className="text-xs text-gray-400 mb-3">Elk bestandstype wordt ondersteund</p>
        <Button
          variant="outline"
          size="sm"
