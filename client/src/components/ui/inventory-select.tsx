...
interface InventorySelectProps {
  ...
  onItemRefreshed?: (item: InventoryItem) => void;
  placeholder?: string;
  testId?: string;
  className?: string;
}
...
export function InventorySelect({
  ...
  const handleRefresh = async () => {
    ...
    if (value && onItemRefreshed) {
      try {
        await queryClient.invalidateQueries({ queryKey: ["/api/inventory", value] });
        const freshItem = await queryClient.fetchQuery<InventoryItem>({
