...
import { Plus, Trash2, CopyPlus, Filter, Search, Check, X, Loader2 } from "lucide-react";
      ...
      <td className="p-2 border-r border-gray-100">
        {editing ? (
          isStandard ? (
            ...
            <Input value={uniqueName} onChange={e => setUniqueName(e.target.value)} placeholder="Naam component..." className="h-8 text-sm" />
          )
        ) : (
      ...
      <td className="p-2 w-20 text-right">
        {editing ? (
          <div className="flex items-center gap-1 justify-end">
            <button onClick={saveRow} disabled={patchMutation.isPending} className="p-1 rounded hover:bg-green-100 text-green-600" title="Opslaan">
          ...
          <div className="flex items-center gap-1 justify-end opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            <button onClick={() => deleteMutation.mutate()} disabled={deleteMutation.isPending} className="p-1 rounded hover:bg-red-50 text-slate-400 hover:text-red-500">
...
interface LineItemAssemblyPanelProps {
  parentLineItemId: string;
  parentLineItemType: string;
  onCostPriceChanged?: (total: number) => void;
}

export function LineItemAssemblyPanel({ parentLineItemId, parentLineItemType, onCostPriceChanged }: LineItemAssemblyPanelProps) {
  const { toast } = useToast();
  const qc = useQueryClient();
      ...
      <div className="bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg px-2 py-1 flex items-center gap-1 mb-3">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" size="sm" className="h-8 w-8 p-0 ring-1 ring-orange-400 text-orange-600" title="Filter">
              <Filter className="h-4 w-4" />
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent>
            <DropdownMenuItem onClick={() => setTypeFilter("all")} className={cn("text-xs", typeFilter === "all" && "font-bold")}>Alle types</DropdownMenuItem>
            <DropdownMenuItem onClick={() => setTypeFilter("standard")} className={cn("text-xs", typeFilter === "standard" && "font-bold")}>Standaard</DropdownMenuItem>
            <DropdownMenuItem onClick={() => setTypeFilter("unique")} className={cn("text-xs", typeFilter === "unique" && "font-bold")}>Uniek</DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" size="sm" className="h-8 w-8 p-0 ring-1 ring-orange-400 text-orange-600" title="Onderdeel toevoegen">
              <Plus className="h-4 w-4" />
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent>
            <DropdownMenuItem onClick={() => addRow("standard")} className="text-xs">Standaard artikel</DropdownMenuItem>
            <DropdownMenuItem onClick={() => addRow("unique")} className="text-xs">Uniek artikel</DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
        <Button
              ...
              <tr className="bg-orange-50 dark:bg-orange-900/20">
                <th className="p-2 w-12 border-r border-orange-200/50" style={{ width: '48px', minWidth: '48px', maxWidth: '48px' }}>
                  <input
                    ...
                    onChange={(e) => {
                      if (e.target.checked) setSelectedRows(filteredComponents.map(c => c.id));
              ...
              {pendingRows.map(row => (
                <tr key={row.tempId} className="border-b border-orange-100 bg-orange-50/40">
                  ...
                  <td className="p-2 w-20">
                    <div className="flex items-center gap-1 justify-end">
                      <button onClick={() => savePending(row)} disabled={createMutation.isPending} className="p-1 rounded hover:bg-green-100 text-green-600" title="Opslaan">
                        {createMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Check className="h-4 w-4" />}
                      </button>
                      <button onClick={() => setPendingRows(prev => prev.filter(r => r.tempId !== row.tempId))} className="p-1 rounded hover:bg-slate-100 text-slate-400" title="Annuleren">
                        <X className="h-4 w-4" />
                      </button>
                    </div>
                  </td>
