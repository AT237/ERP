...
const lineItemFormSchema = insertProformaInvoiceItemSchema.extend({
...
interface ProformaInvoiceLineItemFormLayoutProps {
  onSave: () => void;
  lineItemId?: string;
...
export function ProformaInvoiceLineItemFormLayout({ onSave, lineItemId, proformaInvoiceId, parentId }: ProformaInvoiceLineItemFormLayoutProps) {
  ...
  const [initialCleared] = useState(() => {
    if (!lineItemId) {
      try {
        const key = buildFormPersistenceKey({ formType: "proforma-invoice-line-item", entityId: undefined, scope: proformaInvoiceId });
  ...
  const { data: lineItem, isLoading: isLoadingLineItem } = useQuery<ProformaInvoiceItem>({
    queryKey: ["/api/proforma-invoice-items", lineItemId],
    enabled: !!lineItemId,
  });

  const { data: proformaData } = useQuery<ProformaInvoice>({
    queryKey: ["/api/proforma-invoices", proformaInvoiceId],
    enabled: !!proformaInvoiceId,
  });

  const customerId = proformaData?.customerId;
  ...
  useEffect(() => {
    if (lineItem) {
      const formData: LineItemFormData = {
        proformaInvoiceId: lineItem.proformaInvoiceId || proformaInvoiceId || "",
  ...
  }, [lineItem, form, proformaInvoiceId]);
  ...
  const recordSnippetUsageMutation = useMutation({
    mutationFn: async (data: { snippetId: string; proformaInvoiceId: string; usageType: string }) => {
  ...
  const createMutation = useMutation({
    mutationFn: async (data: LineItemFormData) => {
      const response = await apiRequest("POST", `/api/proforma-invoices/${proformaInvoiceId}/items`, data);
      return response.json();
    },
    onSuccess: (newLineItem) => {
      ...
      const newKey = buildFormPersistenceKey({ formType: "proforma-invoice-line-item", entityId: undefined, scope: proformaInvoiceId });
      localStorage.removeItem(newKey);
      window.dispatchEvent(new CustomEvent('tab-unsaved-changes', {
        detail: { tabId: 'new-proforma-invoice-line-item', hasUnsavedChanges: false }
      }));
      toast({
        title: "Success",
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: LineItemFormData) => {
      const response = await apiRequest("PUT", `/api/proforma-invoice-items/${lineItemId}`, data);
      return response.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/proforma-invoice-items"] });
      queryClient.invalidateQueries({ queryKey: ["/api/proforma-invoice-items", lineItemId] });
      queryClient.invalidateQueries({ queryKey: ["/api/proforma-invoices", proformaInvoiceId, "items"] });
      queryClient.invalidateQueries({ queryKey: ["/api/proforma-invoices", proformaInvoiceId] });
      setHasUnsavedChanges(false);
      const tabId = lineItemId ? `edit-proforma-invoice-line-item-${lineItemId}` : 'new-proforma-invoice-line-item';
  ...
  const handleSelectSnippet = (snippet: TextSnippet) => {
    ...
    if (proformaInvoiceId) {
      recordSnippetUsageMutation.mutate({
        ...
        proformaInvoiceId: proformaInvoiceId,
        usageType: 'proforma-invoice-line-item'
      });
    }
  ...
  const toolbar = useFormToolbar({
    ...
    extraQueryKeysToInvalidate: proformaInvoiceId ? [["/api/proforma-invoices", proformaInvoiceId, "items"], ["/api/proforma-invoices", proformaInvoiceId]] : [],
    navigationListQueryKey: proformaInvoiceId ? ["/api/proforma-invoices", proformaInvoiceId, "items"] : undefined,
    navigationParentId: proformaInvoiceId,
  });
