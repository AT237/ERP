...
export function QuotationRequestFormLayout({ onSave, quotationRequestId, parentId }: QuotationRequestFormLayoutProps) {
  ...
  const qrDirectInput = useMemo(() => ({
    ...
    defaultValues: {
      ...
      hsCode: '',
      countryOfOrigin: '',
      position: String((qrItemsList.length + 1) * 10),
      positionNo: '',
    },
    onAdd: async (values: Record<string, any>) => {
      ...
      await addItemMutation.mutateAsync({
        ...
        ...values,
        position: Number(values.position) || (qrItemsList.length + 1) * 10,
      });
    },
    onUpdate: async (id: string, values: Record<string, any>) => {
      await updateItemMutation.mutateAsync({ id, data: values });
    },
  }), [inventoryOptions, unitOptions, currentId, qrItemsList.length, addItemMutation, updateItemMutation, inventoryItems]);
