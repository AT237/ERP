  ...
  const renderRow = useCallback((row: FormRow<T>, rowIndex: number): ReactNode => {
    switch (row.type) {
      ...
      case 'field':
        ...
        return (
          <div key={field.key as string} className={`grid grid-cols-[130px_1fr] items-center gap-x-6 gap-y-2 ${row.className || ''}`}>
            <Label 
      ...
      case 'fields':
        ...
        return (
          <div key={`fields-${rowIndex}`} className={`grid grid-cols-[130px_1fr] items-center gap-x-6 gap-y-2 ${row.className || ''}`}>
            {/* First field's label */}
            <Label 
            ...
            {row.fields.slice(3).map((field) => {
              ...
              return (
                <div key={field.key as string} className="grid grid-cols-[130px_1fr] items-center gap-x-6 gap-y-2 mt-6">
                  <Label 
                    htmlFor={field.key as string} 
                    className="text-sm font-medium text-right pt-2"
                  >
