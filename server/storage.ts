...
export class DatabaseStorage implements IStorage {
  ...
  async createProspect(prospect: InsertProspect): Promise<Prospect> {
    // Prospect number is now automatically generated by the database
    const values = {
      ...prospect,
      nextFollowUp: prospect.nextFollowUp ? new Date(prospect.nextFollowUp) : null,
      lastContactDate: prospect.lastContactDate ? new Date(prospect.lastContactDate) : null,
      conversionDate: prospect.conversionDate ? new Date(prospect.conversionDate) : null,
    };
    const [newProspect] = await db.insert(prospects).values(values).returning();
    return newProspect;
  }

  async updateProspect(id: string, prospect: Partial<InsertProspect>): Promise<Prospect> {
    const values = {
      ...prospect,
      nextFollowUp: prospect.nextFollowUp ? new Date(prospect.nextFollowUp) : undefined,
  ...
  async convertInvoiceToQuotation(invoiceId: string): Promise<Quotation> {
    ...
    for (const item of invoiceItems) {
      const quotationItem: InsertQuotationItem = {
        quotationId: quotation.id,
        itemId: item.itemId,
        description: item.description,
        quantity: item.quantity,
      ...
      await this.addQuotationItem(quotationItem);
    }
  ...
  async getContractItems(contractId: string): Promise<ContractItem[]> {
    return await db.select().from(contractItems).where(eq(contractItems.contractId, contractId)).orderBy(asc(contractItems.articleNumber));
  }
