...
export async function registerRoutes(app: Express): Promise<Server> {
    ...
    try {
      ...
      const enriched = await Promise.all(components.map(async (comp) => {
        if (comp.componentItemId) {
          try {
            const [inv] = await db.select({ unit: inventoryItems.unit, sku: inventoryItems.sku, name: inventoryItems.name, brand: inventoryItems.brand, manufacturerPartNumber: inventoryItems.manufacturerPartNumber })
              .from(inventoryItems)
  ...
  function sanitizeProformaInvoiceBody(body: any) {
    ...
    const requiredNumerics = ['subtotal', 'totalAmount'];
    requiredNumerics.forEach(f => { if (body[f] === '' || body[f] == null) body[f] = '0'; });
    const nullableFKs = ['customerId', 'projectId', 'paymentDaysId', 'incotermId', 'paymentScheduleId', 'printLayoutId'];
    nullableFKs.forEach(f => { if (body[f] === '') body[f] = null; });
