...
- **Line Item Components (Onderdelen)**: Unique line items (lineType='unique') across quotations, invoices, PFIs, and projects have an "Onderdelen" tab with a sub-component table. Uses `line_item_components` table and `/api/line-item-components/:parentId` API. Reusable `LineItemComponentsPanel` component (`client/src/components/ui/line-item-components-panel.tsx`) supports standard (linked to inventory) and unique (free-text) components, with inline editing, search, filter, duplicate, and bulk delete. Tab only appears when editing an existing unique item.
