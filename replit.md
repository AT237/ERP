...
- **Safe Delete Pattern**: All 14 table/page files use `useEntityDelete` hook (`client/src/hooks/useEntityDelete.ts`) for consistent delete behavior. Row deletes show `SafeDeleteDialog`, bulk deletes handle 409 conflicts via `UsageConflictDialog`. Key entities (customers, suppliers, inventory) have backend usage-check endpoints that hard-block deletion when in use.
